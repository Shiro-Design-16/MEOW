...
const TASK_NAME_USER: &str = "MEOW";
const TASK_NAME_ADMIN: &str = "MEOW (Admin)";
const TASK_XML_DIR: &str = "tasks";
const TASK_XML_USER: &str = "meow-task-user.xml";
const TASK_XML_ADMIN: &str = "meow-task-admin.xml";
...
async fn cleanup_legacy_shortcuts() -> Result<()> {
    ...
    let legacy_spaced_shortcut = startup_dir.join("Clash Verge.lnk");
    let meow_shortcut = startup_dir.join("MEOW.lnk");

    legacy_hyphenated_shortcut.remove_if_exists().await?;
    legacy_spaced_shortcut.remove_if_exists().await?;
    meow_shortcut.remove_if_exists().await?;
    Ok(())
}
