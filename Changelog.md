...
- 新增 macOS 实验性按应用路由：扫描已安装 `.app`，并为选定应用指定 `DIRECT`、`REJECT` 或代理组
...
- macOS 包使用独立的 MEOW 应用标识和数据目录，默认关闭自动更新并移除上游更新源
- 可生成未签名的 macOS 本地测试包；正式 Release 将在图标与签名准备完成后发布

## MEOW 0.0.1（2026-07-19）

> [!NOTE]
> MEOW 是基于 Clash Verge Rev 的非官方独立分支，目前处于品牌定制初始化阶段。本次初始化仅修改项目声明、文档和赞助/推广信息，不包含任何功能行为变更；后续版本可能加入独立功能。

- 将项目文档与贡献指南的主体名称调整为 MEOW
- 增加 fork 来源、非官方关系、问题反馈边界和上游历史归属声明
- 移除上游商业推广、捐助入口、赞助商展示和社区导流
- 建立 `MEOW-0.0.1` Git 标签与 MEOW 自有版本体系
- 将首个版本限定为源码 Release，安装包将在独立应用标识、更新签名与构建流程准备完成后提供
